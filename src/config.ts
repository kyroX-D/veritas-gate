...
/**
 * The bypass switch: `VERITAS_SKIP` in the environment, or `--skip` on the
 * command.
 *
 * Defined once and shared by the CLI and the hook. The promise that there is
 * always a way out is only as good as its weakest copy, so there is only one.
 */
export function isBypassed(argv: readonly string[], env: NodeJS.ProcessEnv): boolean {
...
/**
 * Walks up from `start` to find the directory the project is rooted at.
 *
 * The hook payload's `cwd` is wherever the session happens to be, which is
 * often a subdirectory. Taking it literally means missing the project's
 * `.veritas.yml` and scattering `.veritas/` directories through the tree.
 *
 * A config file wins; otherwise the nearest git repository; otherwise `start`
 * unchanged.
 */
export function findProjectRoot(start: string): string {
  ...
  for (;;) {
    if (findConfigFile(current) !== undefined) {
      return current;
    }
...
/**
 * Quotes a string as YAML.
 *
 * Not JSON.stringify: JSON escapes an embedded double quote as `\"`, which YAML
 * only understands inside a double-quoted scalar with matching unescaping. The
 * mismatch silently rewrote commands such as `node -e "x"` into a different
 * command. Single quotes are preferred because shell commands are full of double
 * quotes and single-quoted YAML only has to escape `'` as `''`.
 */
export function yamlQuote(value: string): string {
  const needsQuoting =
    ...
    /\s#/.test(value) ||
    /[\n\r\t]/.test(value) ||
    ["true", "false", "yes", "no", "on", "off", "null", "~"].includes(value.toLowerCase()) ||
    /^-?\d+(\.\d+)?$/.test(value);
...
export function renderConfig(config: Config, detection?: Detection): string {
  ...
  lines.push("# https://github.com/kyroX-D/veritas-gate");
